...
u8  HydrogenProducerTxBuf[REMOTE_HP_TX_BUFF_SIZE]; //制氢半机实时数据
u8  FuelCellTxBuf[REMOTE_FC_TX_BUFF_SIZE]; //发电半机实时数据
u8  Uart2RxBuf[PRGM_3G_RX_BUFF_SIZE];//注意变量数组不要溢出
...
static void LoadFuelCellRealTimeInfo(void)
{
    ...
    FuelCellTxBuf[25] = 0;
    FuelCellTxBuf[26] = 0x4E;
    FuelCellTxBuf[27] = 0x54;
    FuelCellTxBuf[28] = 0x30;
    FuelCellTxBuf[29] = 0x30;
    FuelCellTxBuf[30] = 0x39;
    FuelCellTxBuf[31] = 0x39;
...
//        }
		memset(send_buff, 0, REMOTE_TX_BUFF_SIZE);
