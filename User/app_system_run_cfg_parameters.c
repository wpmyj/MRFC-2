...
//0x0801FC00
#define SYS_PARA_STORE_START_ADDR         0x0803F800      //地址頂端為0x0803FFFF 设置FLASH 保存地址為最後的2K字節(必须为偶数)
...
#define	MAX_VI_LIMIT_VALUE_PARA_STORE_OFFSET_ADDR								170	//最大限流限压值
#define MACHINE_ID_PARA_STORE_OFFSET_ADDR										176	//机器远程ID
/*
***************************************************************************************************
*                                     GLOBAL  VARIABLES
***************************************************************************************************
...
MAX_VI_VALUE_PARA_Typedef								g_stMaxVILimitPara;//最大限流限压参数
...
static void GetParametersSelectFlag(uint16_t *o_pParametersSelectFlagAddr)
{
    STMFLASH_Read(SYS_PARA_STORE_START_ADDR + PARAMETERS_SELECT_FLAG_OFFSET_ADDR, o_pParametersSelectFlagAddr, 1);//获取运行参数选择标志
}
...
static void GetSystemWorkTimesFromFlash(u16 *o_StoreAddr)
{
    STMFLASH_Read(SYS_PARA_STORE_START_ADDR + SYSTEM_RUN_TIME_STORE_OFFSET_ADDR, o_StoreAddr, 1);//获取运行次数
}
...
static void GetDefaultStartHydrgPumpSpdPara(HYDROGEN_PUMP_SPEED_PARA_Typedef *i_StartPumpSpdPara)
{
    ...
    i_StartPumpSpdPara->PumpSpdAfterLiquidPressExceed4Kg  = 380;//稳定运行的泵速
}
