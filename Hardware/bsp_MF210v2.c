...
#if(DEBUG_3G)
    #define SERVER_IP_PORT   "112.74.132.34,40399"  //for debug
#else
    #define SERVER_IP_PORT   "120.76.204.115,40304"	//use
#endif
...
bool module_control(const char *send, const char *ask, int *ret)
{
    ...
    do
    {
         ...
         if(find_ret != NULL)
        {
            if(strlen(ask) != strlen(index_string)) // 如果ask和index_string的长度不相等，说明askz存在返回值
            {
                ...
                sscanf(find_ret, "%d", ret);
//                DEBUG_PRINF("result = %d find_ret = %s\r\n", result, find_ret);
//                find_ret += strlen(index_string);
...
{
    char *tmp = strstr((const char *)usart2_recv_buff, "+ZIPRECV:");//判断+ZIPRECV:是否在接收字符中
    ...
    if(tmp != NULL)
    {
        ...
        if(strchr(tmp, 0x0a))
        {
            sscanf(tmp, "+ZIPRECV: 1,"SERVER_IP_PORT",%d,%s", &length, recv_str);//将tmp数组中按格式将数据提取到length和recv_str中
