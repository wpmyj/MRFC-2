...
uint8_t g_u8WifiCmdRec = DEF_NO;
uint8_t g_u83GCommandReceived = DEF_NO;
/*
***************************************************************************************************
...
static 	void 		BSP_Ser3_ISR_Handler(void);
...
CPU_INT08U  BSP_Ser_RdByteUnlocked(void)
{
    ...
    USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);              /* Enable the Receive not empty interrupt             */

    BSP_OS_SemWait(&BSP_SerRxWait, 0);                          /* Wait until data is received                        */

    USART_ITConfig(USART1, USART_IT_RXNE, DISABLE);             /* Disable the Receive not empty interrupt            */
...
static void  BSP_SerTo3G_ISR_Handler(void)
{
    ...
    if(rxne_status == SET) {
        ...
        if(recv_cursor < 1024) {
            usart2_recv_buff[recv_cursor] = BSP_Ser2RxData;   //记录接收到的值
            recv_cursor++;
        } else {
            recv_cursor = 0;
        }
...
***************************************************************************************************
*/
void Uart2SendString(const char *s)
{
    while(*s) {
...
{
    SocketSend(SOCKET_ID, buffer, count);
}
...
*                             Bsp_Usart3Init()
*
* Description :Init the usart3 which use to set the remote id and cueernt and voletage limit point.
*
* Arguments   : none.
*
* Returns     : none.
...
void Bsp_Usart3Init(void)
{
    ...
    BSP_IntVectSet(BSP_INT_ID_USART3, BSP_Ser3_ISR_Handler);
...
void Uart3SendData(u8 *buf, u8 len)
{
    uint8_t i = 0;
...
static void BSP_Ser3_ISR_Handler(void)
{
    ...
    if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET) {
		chr = (USART_ReceiveData(USART3) & 0xFF); //读取接收到的数据
        recLocalCfgData(chr);
        processData();        
...
void  BSP_SerToWIFI_Init()
{
	...
	BSP_SerToWIFI_RxMsgInit(GetPrgmRxBuffAddr(), GetPrgmRxBuffLen());//串口接收初始化
}
...
void BSP_PrgmDataDMASend(uint8_t i_u8TxBuffSize, uint8_t *TxBuff)
{
    ...
    BSP_OS_TimeDlyMs(20);//等待发送完成再进行下一次发送初始化
}
...
void WIFI_DataRx_IRQHandler(void)
{
    ...
    if(DMA_GetITStatus(DMA2_IT_TC3) != RESET) {
        if(g_u8WifiCmdRec == NO) {
            CPU_CRITICAL_ENTER();
            OSTimeDlyResume(&CommTaskTCB,&err);
            g_u8WifiCmdRec = YES;
            CPU_CRITICAL_EXIT();
        }
